...
const ChatMessage = ({ message, onFeedback, showWhatsApp = false }) => {
  ...
  const debouncedLike = useDebouncedClick(
    () => onFeedback?.(message.id, "like"),
    350,
  );
  const debouncedDislike = useDebouncedClick(
  ...
  const shouldShowWhatsApp =
    isBot &&
    (showWhatsApp || message.content?.toLowerCase()?.includes("whatsapp"));

  return (
    <div
      className={`flex mb-6 ${isUser ? "justify-end" : "justify-start"} group animate-in fade-in slide-in-from-bottom-2`}
    >
      {/* Bot Icon */}
      {isBot && (
          ...
          <div className="text-sm prose prose-sm max-w-none break-words leading-relaxed">
            <ReactMarkdown
              ...
              components={{
                ...
                p: ({ children }) => (
                  <p className="mb-0 last:mb-0 whitespace-pre-wrap">
                    {children}
                  </p>
                ),
                ul: ({ children }) => (
